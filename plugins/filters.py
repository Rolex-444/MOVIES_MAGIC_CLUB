...
# ✅ Group search with TEXT LINKS (FULL name visible!)
@Client.on_message(filters.text & filters.group, group=1)
async def group_search_handler(client, message):
    """Handle movie search in GROUPS with clickable TEXT links"""
    ...
    try:
        ...
        file_text = f"📁 **Found {total} All files for `{search}`**\n\n"
        
        # ✅ Build TEXT LINKS with FULL file names (NOT buttons!)
        for file in page_files:
            try:
                ...
                # ✅ Create clickable MARKDOWN link (FULL name visible!)
                deep_link = f"https://t.me/{bot_username}?start=file_{file_id}"
                file_text += f"📁 {file_size} ▷ [{cleaned_name}]({deep_link})\n\n"
        ...
        # ✅ Build ONLY filter buttons (NO file buttons)
        buttons = [
        ...
        logger.info(f"✅ Search results sent to group {message.chat.id} with TEXT links")
    except Exception as e:
        logger.error(f"❌ Error in group_search: {e}", exc_info=True)

# ✅ Private search with TEXT LINKS (FULL name visible!)
...
async def private_search(client, message):
    """Handle movie search in PRIVATE chat with clickable TEXT links"""
    search = message.text.strip()
...
# ✅ Handle 3 download button options
...
logger.info("✅ FILTERS PLUGIN LOADED WITH TEXT LINKS (FULL FILE NAMES VISIBLE)!")
