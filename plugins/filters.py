...
def get_filter_info(user_id):
    ...
    qual = filters.get('quality', 'All')
    seas = filters.get('season', 'All')
...
def apply_user_filters(files, user_id):
    ...
    if filters.get('language') and filters['language'] != 'All':
        filtered = [f for f in filtered if filters['language'].lower() in f.get('file_name', '').lower()]
    
    # Filter by quality
    if filters.get('quality') and filters['quality'] != 'All':
        filtered = [f for f in filtered if filters['quality'].lower() in f.get('file_name', '').lower()]
    
    # Filter by season
    if filters.get('season') and filters['season'] != 'All':
        filtered = [f for f in filtered if filters['season'].lower() in f.get('file_name', '').lower()]
